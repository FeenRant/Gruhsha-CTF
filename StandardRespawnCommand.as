...
//
// Gruhsha: Changes are on 94 line
		...
		case SpawnCmd::changeClass:
		{
			if (getNet().isServer())
			{
				...
				if (caller !is null && canChangeClass(this, caller))
				{
					...
					{
						int playerTeamID = caller.getTeamNum();
						int thisTeamBuildersCount = 0;
						int maxBuilders = 1;
						...
						for (int i = 0; i < getPlayerCount(); i++) 
						{
							CPlayer@ player = getPlayer(i);
							{
								...
								if (player.getTeamNum() == playerTeamID && player.getBlob().getName() == "builder" && !rules.isBarrier()) 
								{
									thisTeamBuildersCount++;
									if (thisTeamBuildersCount >= maxBuilders) return;
								}
							}
						}
					}
					
					//end of added part of code
